...
namespace Password_Philosophy
{
    class Program
    {
        static void Main()
        {
            static void SolveTask(List<string> list)
            {
                int validCount = 0;

                foreach (var item in list)
                {
                    string rule = item.Split(':')[0];

                    string password = item.Split(':')[1].Trim();

                    string ruleRange = rule.Split(' ')[0];
                    string ruleLetter = rule.Split(' ')[1];

                    int minimumTimes = Convert.ToInt32(ruleRange.Split('-')[0]);
                    int maximumTimes = Convert.ToInt32(ruleRange.Split('-')[1]);

                    int letterCount = Regex.Matches(password, ruleLetter).Count;

                    if (letterCount >= minimumTimes && letterCount <= maximumTimes)
                    {
                        validCount++;
                    }
                }

                Console.WriteLine($"Valid password count is: {validCount}");
            }
